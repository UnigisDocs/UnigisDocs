...
.. raw:: html
   ...
   <input type="text" id="searchInput" onkeyup="filterQuestions()" placeholder="Buscar preguntas..." style="width: 100%; padding: 10px; margin-bottom: 20px;">
...
.. raw:: html
 ...
 <ul>
  <li><a href="#mapi" class="special-link">MAPI</a></li>
  <li><a href="#alarmas" class="special-link">Alarmas</a></li>
  <li><a href="#tracking" class="special-link">Tracking</a></li>
  <li><a href="#planning" class="special-link">Planning</a></li>
  <li><a href="#mobile" class="special-link">Mobile</a></li>
  <li><a href="#setup" class="special-link">SetUp</a></li>
 </ul>
...
.. raw:: html
  ...
  <details class="question">
    <summary class="card-header">¿Como funciona el geotipo Grupo de Rutas y cuál es la diferencia con Grupo de Rutas Inclusivo?</summary>
  ...
  <details class="question">
    <summary class="card-header">¿Como funciona el reordenamiento automático de Rutas?</summary>
...
.. raw:: html
  ...
  function filterQuestions() {
    // Declare variables
    var input, filter, details, i, txtValue, keywords;
    input = document.getElementById('searchInput');
    filter = input.value.toUpperCase();
    keywords = filter.split(' ').filter(word => word.length > 0); // Split search input into keywords

    details = document.getElementsByClassName('question');

    // Loop through all FAQ sections
    for (i = 0; i < details.length; i++) {
        // Check if the current question matches any keyword
        txtValue = details[i].textContent || details[i].innerText;
        var matchesKeyword = keywords.every(keyword => txtValue.toUpperCase().includes(keyword));
